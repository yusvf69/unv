...
export default function Students() {
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <motion.h1 initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-3xl font-serif font-bold mb-2">طلاب الكلية</motion.h1>
      <p className="text-sm text-muted-foreground mb-6">تابع أصدقاءك وزملاءك وابدأ محادثة معهم</p>

      <div className="relative mb-6 max-w-md">
        <Search className="absolute start-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="ابحث بالاسم أو التخصص..." className="ps-9" />
      </div>
        ...
        {filtered.map((s, i) => (
          ...
          >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1 mt-1">
                    ...
                    {s.yearInCollege && <span className="text-[10px] bg-secondary/10 text-secondary px-1.5 py-0.5 rounded font-bold">سنة {s.yearInCollege}</span>}
            ...
            <div className="flex gap-2 mt-3">
              ...
              >
                {followingIds.has(s.id) ? <><UserCheck className="me-1 h-3.5 w-3.5" /> أتابع</> : <><UserPlus className="me-1 h-3.5 w-3.5" /> متابعة</>}
              </Button>
      ...
      {!filtered.length && <p className="text-center text-muted-foreground py-12">لا يوجد طلاب مطابقون. سجّل أصدقاءك ليظهروا هنا.</p>}
    </div>
  );
}
