  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-5xl">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 flex items-start justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-3xl font-serif font-bold">إدارة الاختبارات</h1>
          <p className="text-sm text-muted-foreground mt-1">
            أنشئ اختبارات مخصصة بمدة ونقاط محددة، وأضف أسئلة اختيار من متعدد وصح وخطأ
          </p>
        </div>
        <Button onClick={() => setCreating(true)}><Plus className="me-2 h-4 w-4" /> إنشاء اختبار</Button>
      </motion.div>
      ...
      {!isLoading && !quizzes.length && (
        <div className="text-center py-16 bg-card border rounded-2xl">
          ...
          <p className="text-muted-foreground">لا توجد اختبارات بعد. أنشئ أول اختبار!</p>
        </div>
      )}

      <div className="space-y-3">
        {quizzes.map((q) => (
          <motion.div key={q.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="bg-card border rounded-2xl p-4 flex items-center gap-4 flex-wrap">
            <div className="flex-1 min-w-[240px]">
              <div className="flex items-center gap-2 flex-wrap">
                <h3 className="font-bold text-lg">{q.title}</h3>
                <span className={`text-xs px-2 py-0.5 rounded-full font-bold ${q.isOpen ? "bg-emerald-500/20 text-emerald-700" : "bg-muted text-muted-foreground"}`}>
                  {q.isOpen ? "مفتوح" : "مغلق"}
                </span>
                <span className="text-xs bg-secondary/10 text-secondary px-2 py-0.5 rounded-full">{q.difficulty}</span>
                {q.groupOnly && <span className="text-xs bg-primary/10 text-primary px-2 py-0.5 rounded-full font-bold">G{q.groupOnly}</span>}
                {q.yearOnly && <span className="text-xs bg-amber-500/10 text-amber-700 px-2 py-0.5 rounded-full">سنة {q.yearOnly}</span>}
              </div>
              <p className="text-sm text-muted-foreground mt-1">{q.courseTitle}</p>
            ...
            <div className="flex gap-2 flex-wrap">
              <Button size="sm" variant="outline" onClick={() => setManagingQuestions(q.id)}>
                <Settings className="me-1 h-4 w-4" /> الأسئلة
              </Button>
              <Button size="sm" variant="outline" onClick={() => setViewing(q.id)}>
                <Eye className="me-1 h-4 w-4" /> المحاولات
              </Button>
              <Button
                size="sm"
              ...
              >
                {q.isOpen ? <><XCircle className="me-1 h-4 w-4" /> إغلاق</> : <><CheckCircle className="me-1 h-4 w-4" /> فتح</>}
              </Button>
              <Button size="sm" variant="ghost" onClick={() => setEditing(q)}><Edit2 className="h-4 w-4" /></Button>
      ...
      <Dialog open={!!viewing} onOpenChange={(v) => !v && setViewing(null)}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>محاولات الطلاب</DialogTitle></DialogHeader>
          {attempts.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">لا توجد محاولات بعد</p>
          ) : (
              ...
              {attempts.map((a) => {
                ...
                return (
                  <div key={a.id} className="flex items-center gap-3 p-3 border rounded-lg">
                    {a.userAvatar ? (
                      <img src={a.userAvatar} alt="" className="w-10 h-10 rounded-full object-cover" />
                    ) : (
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary">{a.userName?.charAt(0)}</div>
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="font-bold text-sm truncate">{a.userName || `User #${a.userId}`}</div>
                      <div className="text-xs text-muted-foreground">{new Date(a.completedAt).toLocaleString("ar-EG")} · {a.userGroup ? `G${a.userGroup}` : ""}</div>
                    </div>
                    <div className={`text-sm font-bold ${pct >= 80 ? "text-emerald-600" : pct >= 50 ? "text-amber-600" : "text-destructive"}`}>
                      {a.score}/{a.total} ({pct}%)
                    </div>
                    <Button size="sm" variant="ghost" onClick={() => setViewingAttempt(a.id)}>
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>تفاصيل محاولة الطالب</DialogTitle>
        </DialogHeader>
        ...
        {detail && (
          <>
            ...
            <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-xl">
              {detail.userAvatar ? (
                <img src={detail.userAvatar} alt="" className="w-12 h-12 rounded-full object-cover" />
              ) : (
                <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center font-bold text-primary text-lg">{detail.userName?.charAt(0)}</div>
              )}
              <div className="flex-1">
                <div className="font-bold">{detail.userName || "مستخدم محذوف"}</div>
                <div className="text-xs text-muted-foreground">{new Date(detail.completedAt).toLocaleString("ar-EG")} · المدة: {formatDuration(detail.durationSec)}</div>
              </div>
              <div className="text-center">
                <div className={`text-2xl font-bold ${detail.passed ? "text-emerald-600" : "text-destructive"}`}>{pct}%</div>
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="bg-emerald-500/10 rounded-lg p-3">
                <div className="text-xl font-bold text-emerald-600">{detail.questions.filter((q) => q.correct).length}</div>
                <div className="text-[10px] text-muted-foreground">إجابة صحيحة</div>
              </div>
              <div className="bg-red-500/10 rounded-lg p-3">
                <div className="text-xl font-bold text-destructive">{detail.questions.filter((q) => !q.correct).length}</div>
                <div className="text-[10px] text-muted-foreground">إجابة خاطئة</div>
              </div>
              <div className="bg-amber-500/10 rounded-lg p-3">
                <div className="text-xl font-bold text-amber-600">{detail.score}/{detail.total}</div>
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          <div>
            ...
            <Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="اختبار منتصف الفصل" />
          </div>
          <div>
            <Label className="text-xs">الوصف</Label>
            <Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="وصف الاختبار..." rows={2} />
          </div>
          <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            <div>
              ...
              <Input type="number" value={form.passPercent} onChange={(e) => setForm({ ...form, passPercent: Number(e.target.value) })} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label className="text-xs">المستوى</Label>
              <select value={form.difficulty} onChange={(e) => setForm({ ...form, difficulty: e.target.value })} className="w-full h-10 px-3 border-2 border-input rounded-md bg-background text-sm">
            ...
            <div>
              ...
              <Input type="number" value={form.yearOnly || ""} onChange={(e) => setForm({ ...form, yearOnly: Number(e.target.value) || 0 })} placeholder="0 = الكل" />
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <Label htmlFor="randomize" className="text-sm">خلط ترتيب الأسئلة</Label>
          </div>
        </div>
        <DialogFooter>
          <Button variant="ghost" onClick={onClose}>إلغاء</Button>
          <Button onClick={submit} disabled={create.isPending}>
            {create.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><Send className="me-2 h-4 w-4" /> إنشاء</>}
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
function EditQuizDialog({ quiz, onClose }: { quiz: any; onClose: () => void }) {
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent>
        <DialogHeader><DialogTitle>تعديل الاختبار</DialogTitle></DialogHeader>
        <div className="space-y-3">
          <div><Label className="text-xs">العنوان</Label><Input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} /></div>
          <div><Label className="text-xs">الوصف</Label><Textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={2} /></div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <div><Label className="text-xs">المدة (دقائق)</Label><Input type="number" value={form.durationMinutes} onChange={(e) => setForm({ ...form, durationMinutes: Number(e.target.value) })} /></div>
            <div><Label className="text-xs">النقاط</Label><Input type="number" value={form.totalPoints} onChange={(e) => setForm({ ...form, totalPoints: Number(e.target.value) })} /></div>
            <div><Label className="text-xs">نسبة النجاح %</Label><Input type="number" value={form.passPercent} onChange={(e) => setForm({ ...form, passPercent: Number(e.target.value) })} /></div>
          </div>
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        {!adding ? (
          <Button onClick={() => setAdding(true)} className="mb-3"><Plus className="me-2 h-4 w-4" /> إضافة سؤال</Button>
        ) : (
          <div className="space-y-3 p-3 border rounded-xl mb-3 bg-muted/20">
            <div className="grid grid-cols-2 gap-3">
              <div><Label className="text-xs">نوع السؤال</Label>
                <select value={newQ.type} onChange={(e) => setNewQ({ ...newQ, type: e.target.value as "mc" | "tf" })} className="w-full h-10 px-3 border-2 border-input rounded-md bg-background text-sm">
                  <option value="mc">اختيار من متعدد</option>
                  <option value="tf">صح / خطأ</option>
                </select>
              </div>
              <div><Label className="text-xs">النقاط</Label><Input type="number" value={newQ.points} onChange={(e) => setNewQ({ ...newQ, points: Number(e.target.value) })} /></div>
            </div>
            <div><Label className="text-xs">نص السؤال</Label><Textarea value={newQ.text} onChange={(e) => setNewQ({ ...newQ, text: e.target.value })} placeholder="اكتب السؤال..." /></div>
            {newQ.type === "mc" && (
              <div className="space-y-2">
                ...
                {newQ.options.map((opt, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <Input value={opt} onChange={(e) => { const opts = [...newQ.options]; opts[i] = e.target.value; setNewQ({ ...newQ, options: opts }); }} placeholder={`الخيار ${i + 1}`} />
                  </div>
                ))}
              </div>
            )}
            {newQ.type === "tf" && (
              <div className="flex gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-bold text-green-600">صح</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <span className="text-sm font-bold text-destructive">خطأ</span>
                </label>
              </div>
            )}
            <div><Label className="text-xs">الشرح (اختياري)</Label><Textarea value={newQ.explanation} onChange={(e) => setNewQ({ ...newQ, explanation: e.target.value })} placeholder="شرح الإجابة..." rows={1} /></div>
            <div className="flex gap-2">
              <Button variant="outline" onClick={() => setAdding(false)}>إلغاء</Button>
              <Button onClick={submit} disabled={addQ.isPending}>{addQ.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "إضافة"}</Button>
            </div>
          </div>
        )}
